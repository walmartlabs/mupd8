...
object MessageParser extends App {
  ...
  def getMessage(msg: String): Message = {
    ...
    messageKind match {
      case "HOST_REQUEST" => new HostRequestMessage(msgContent)
      case "HOST_LIST" => HostListMessage.initFromString(msgContent)
      case "NODE_FAILURE" =>
      ...
      case "LOAD_DIST_INSTRUCT" =>
        LoadDistInstructMessage.initFromString(msgContent)
      case "UPDATE_RING" =>
        UpdateRingMessage.initFromString(msgContent)
    }
