...
class MessageServerTest extends FunSuite {
  ...
  test("MessageServer/Client add/remove") {
    ...
    for (node <- nodes) client.sendMessage(NodeChangeMessage(Set(node), Set.empty, "test"))
    Thread.sleep(1000)
    assert(server.ring.size == 5)
    for (node <- nodes) client.sendMessage(NodeChangeMessage(Set.empty, Set(node), "test"))
    Thread.sleep(1000)
